...
export async function alertsRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/api/alerts/:alert_id/acknowledge', { preHandler: [requirePermission('alerts:write')] }, async (request, reply) => {
    ...
    try {
      const res = await pool.query(`UPDATE alerts SET status = 'acknowledged', acknowledged_at = CURRENT_TIMESTAMP, acknowledged_by = $1 WHERE alert_id = $2 AND status = 'new' RETURNING *`, [context.user_id, alert_id]);
  ...
  fastify.post('/api/alerts/:alert_id/assign', { preHandler: [requirePermission('alerts:write')] }, async (request, reply) => {
    ...
    if (!pool) return reply.status(503).send({ error: 'Database unavailable' });
    try {
      const res = await pool.query(`UPDATE alerts SET status = 'assigned', assignee_id = $1 WHERE alert_id = $2 AND status IN ('new', 'acknowledged') RETURNING *`, [assignee_id, alert_id]);
      if (res.rowCount === 0) return reply.status(404).send({ error: 'Not Found or invalid state' });
  ...
  fastify.post('/api/alerts/:alert_id/investigate', { preHandler: [requirePermission('alerts:write')] }, async (request, reply) => {
    ...
    if (!pool) return reply.status(503).send({ error: 'Database unavailable' });
    try {
      const res = await pool.query(`UPDATE alerts SET status = 'investigating', investigating_at = CURRENT_TIMESTAMP, investigating_by = $1 WHERE alert_id = $2 AND status IN ('new', 'acknowledged', 'assigned') RETURNING *`, [context.user_id, alert_id]);
      if (res.rowCount === 0) return reply.status(404).send({ error: 'Not Found or invalid state' });
  ...
  fastify.post('/api/alerts/:alert_id/resolve', { preHandler: [requirePermission('alerts:write')] }, async (request, reply) => {
    ...
    if (!pool) return reply.status(503).send({ error: 'Database unavailable' });
    try {
      const res = await pool.query(`UPDATE alerts SET status = 'resolved', resolved_at = CURRENT_TIMESTAMP, resolved_by = $1, resolution_notes = $2 WHERE alert_id = $3 AND status != 'resolved' RETURNING *`, [context.user_id, notes, alert_id]);
      if (res.rowCount === 0) return reply.status(404).send({ error: 'Not Found or invalid state' });
  ...
  fastify.post('/api/alerts/:alert_id/reopen', { preHandler: [requirePermission('alerts:write')] }, async (request, reply) => {
    ...
    if (!pool) return reply.status(503).send({ error: 'Database unavailable' });
    try {
      const res = await pool.query(`UPDATE alerts SET status = 'new', resolved_at = NULL, resolved_by = NULL, resolution_notes = NULL WHERE alert_id = $1 AND status = 'resolved' RETURNING *`, [alert_id]);
      if (res.rowCount === 0) return reply.status(404).send({ error: 'Not Found or invalid state' });
